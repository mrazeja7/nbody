...
void simulate()
{
	...
	uint64_t appxFlops = (19*sim.getCount()-5)*sim.getCount()*k;
	timeInSeconds = finish;
	float gflops = 1e-9 * appxFlops / finish;	

	cout << sim.getCount() << /*" bodies\t"*/"\t" << sim.getCurrentTime() << /*" iterations\t"*/"\t" << timeInSeconds << /*" seconds\t"*/"\t" << gflops /*<< " GFlops/s." */<< endl;	
	//cout << sim.getCount() << " bodies\n" << sim.getCurrentTime() << " iterations\n" << timeInSeconds << " seconds\n" << gflops << " GFlops/s." << endl;
}
